...
server.registerTool(
  ...
  {
    description:
      "Make one decision: move the cursor to the option with this label (exact match after trimming/case-folding; an option's `name` matches too, e.g. `Great Ball` for `Great Ball ×9`) and commit with ACTION. Returns messages crossed while auto-advancing text, the lean snapshot and the next menu. Refuses (nothing pressed) on no_match, ambiguous, screen_changed, settings screens, the starter filter bar, a contended tab or a frozen loop. To leave a screen, select the option that leaves it (Cancel, No): there is no back button. On MODIFIER_SELECT, taking a free reward (cost 0) ends the shop and advances to the next wave: buy shop items (cost > 0) first.",
    inputSchema: {
