...
import { foreignPort, hubWontStart, offeredCommands, reachLadder, readyTabs } from "./reach.ts";
...
test("rung 4: a browser below the protocol window is too old, one above it means the plugin is", () => {
  const old = reachLadder(state([browser({ protocol: PROTOCOL - 2 })], [tab()]));
  assert.deepEqual(old, { rung: 4, line: "Coachemon 1.2.0 in Chrome is too old for this plugin. Update the extension." });
  const newer = reachLadder(state([browser({ protocol: PROTOCOL + 1, target: "firefox" })], [tab({ target: "firefox" })]));
...
test("rung 4 also covers a command this call needs that the browser never listed (§8.5)", () => {
  const r = reachLadder(state([browser({ commands: ["probe", "menu"] })], [tab()]), ["probe", "menu", "snapshot"]);
  ...
  assert.equal(reachLadder(state([browser({ commands: ["probe", "menu"] })], [tab()]), ["probe", "menu"]), null);
});

test("rung 5: Firefox connected without consent names the click that fixes it (§8.4)", () => {
  const r = reachLadder(state([browser({ target: "firefox", consent: false })], [tab({ target: "firefox" })]));
  ...
  assert.notEqual(reachLadder(state([browser({ consent: false })], [tab()]))?.rung, 5);
});

test("rung 6: a tab whose page scripts ran isolated means the browser is too old (§9.7)", () => {
  const r = reachLadder(state([browser({ target: "safari" })], [tab({ target: "safari", state: "wrong-world" })]));
...
test("rung 7: connected, but nothing is ready to play", () => {
  const r = reachLadder(state([browser()], []));
...
test("one ready tab on a browser that is up to date is reachable: no rung at all", () => {
  assert.equal(reachLadder(state([browser()], [tab()]), COMMANDS), null);
});
...
test("a tab on a browser without consent is not counted, so it reads as rung 5 rather than as a second tab (§7.5)", () => {
  ...
  assert.equal(reachLadder(s)?.rung, 5);
});
