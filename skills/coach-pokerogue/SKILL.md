...
1. **HUD** — `scripts/read.sh <browser> hud` draws a panel in the game tab's top-left corner and refreshes every second with no Claude involved. Source is `scripts/hud/*.js`, bundled by `hud-bundle.mjs`; behaviour is pinned by `node scripts/test/run.mjs`. `hud-off` removes it. Header buttons switch **full** / **mini** / **closed**.
   ...
   - **🩺 Team audit** (on the rewards card): what is wrong with how the team is built, while a shop, TM or Memory Mushroom can still fix it. One line `🩺 team: N issues · <worst> [+]`; full view opens it by itself when the next big fight's roster has a single answer or a speed gap. `49-audit.js`, `references/game-code.md` §17; every threshold is a first cut.
   ...
   - **🗺 Biome card** (the next-biome choice a Map offers): ranks each biome for the party with a score and ★ pick / ≈ close. Encounters come from the game's own biome pools, wave by wave over the next ten: the wild tier odds with party luck and a Daily forced tier, time of day, the trainer odds and the biome's trainer classes with their parties, the tenth wave's wild boss or gym leader, fixed battles left out, species evolved at the party's level by the game's thresholds. ✓/✗ who's weak, who resists, how many mons hit them SE, the gym leader ahead (`W50 gym Poison (Janine): 2 hit SE`), and a 🎯 catch that covers a weakness, outclasses the weakest member or is new. The tenth wave also counts on its own (± 10), and a near tie names what decided it (`edges Cave on defense`). The fainted count (the X1 heal revives them) except under Hardcore or a no-heal Limited Support. Full view adds the type mix, the species met most, 👤 the trainers met, 👑 the wild boss and where the biome leads (★ rare: Space, Fairy Cave, Laboratory). The tables are read from the game's loaded modules a moment after the HUD starts; until then the card only lists the options.
