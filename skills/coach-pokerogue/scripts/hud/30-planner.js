...
const fieldPlan = (s, party, active, double, attackers = active, { freeSwitch = false, locked = null } = {}) => {
  ...
  const options = (me, entering, mine = null) => {
    ...
    const one = (o, fi) => {
      ...
      return { me, move: o, target: fi, turns, hits: x.turnsWe, score: danger - turns + (x.pWeKoFirst - x.pTheyKoFirst) - cost(o), hp };
    };
      ...
      slots: best.picks.map((p, i) => {
        ...
        return {
          ...
          notes: sup ? [sup.note] : [
            ...
            ...(each ? active.map((f, fi) => each[fi] <= 3 && `${f.name} ${each[fi]}HKO`).filter(Boolean) : []),
            ...notesFor(p),
