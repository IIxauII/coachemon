...
// Mini: one line per target — `🎯 catch: [ball] 78% — new species, covers Ground weakness`. Full adds the odds of
// every ball we hold and the reasons.
const drawCatch = m => {
  const c = m?.targets ? m : m?.catch;
  if (!c?.targets?.length || view === "closed") return [];
  const COLOR = { catch: "#6d6", maybe: "#ec4", skip: "#9aa" };
  const KIND = { account: ["★", "#c9f"], team: ["⚑", "#8cf"], escape: ["⇥", "#fa4"] };
  // Ball sprites live in the `items` atlas; a short name stands in until it's loaded.
  const ball = (x, height = 14) => img("items", x.key, x.ball, height, null) || h("span", { fontSize: "9px", marginRight: "1px" }, x.short);
  const pct = p => `${Math.round(p * 100)}%`;
  const out = view === "full" ? [h("div", { borderTop: "1px solid rgba(255,255,255,.12)", margin: "5px 0 2px" })] : [];
  for (const t of c.targets) {
    const color = COLOR[t.verdict];
    const best = t.verdict !== "skip" && t.best;
    out.push(line("🎯", color,
      ...
      h("span", t.verdict === "skip" ? dim : {}, best ? `— ${t.why}` : t.why)));
    ...
    out.push(line("", "#9aa", ...t.chance.map(x => h("span", { display: "inline-flex", alignItems: "center", marginRight: "5px", ...(x.p > 0 ? {} : dim) },
      ball(x, 12), h("span", { fontSize: "9px" }, `×${x.count} ${pct(x.p)}`)))));
