...
const cardNow = () => {
  try {
    ...
    return ev && { ...ev, text: cardText(card) };
  } catch {
    return null;
  }
};
...
const stream = () => {
  ...
  if (!ev || CARD_KINDS.indexOf(ev.kind) < 0 || typeof ev.wave !== "number" || typeof ev.key !== "string"
    || typeof ev.verdict !== "string" || typeof ev.text !== "string") { sentCard = null; return; }
  const sig = `${ev.kind}|${ev.key}|${ev.verdict}`;
  if (sig === sentCard) return;
  sentCard = sig;
  push(CARD_EVENT, { kind: ev.kind, key: ev.key, wave: ev.wave, verdict: ev.verdict, text: ev.text });
};
