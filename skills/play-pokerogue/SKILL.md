...
- Refusals (`no_match`, `ambiguous`, `screen_changed`, `game_moved`, …) pressed nothing: `read_menu` and retry with the right label or `index`. `message_pending` is different: a message is up (e.g. "It won't have any effect."), so `press(ACTION)` first, then retry.
