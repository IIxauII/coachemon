...
- `isCoverage(mv)` → whether one move hits for damage off the type chart, the rule the two below are built on and the one rule for both sides of the field: the wave preview filters a **foe**'s moveset with it (48-preview's `attacks`), where it read `power > 0` until #266 and so lost a Steel foe whose STAB is Gyro Ball.
...
- **A typing written onto the foe** (Soak, Magic Powder, Forest's Curse, Trick-or-Treat): worth what it opens for the
  ...
  not of its types: the preview's `attacks` carries one entry per attacking move, so three Steel moves beside one
