...
| Safari Zone | money ≥ ×2 (`:57`) | pay ×2 → a **continuous encounter**: three wild mons in turn, each with ball / bait / mud / run as `overrideOptions`. Catch rate is `round(speciesCatchRate × 1.5 × (2 + max(catchStage, 0)) / (2 − min(catchStage, 0)))`, and that rate is **not** a probability: `throwPokeball` turns it into `ballTwitchRate = round(1048560 / ⁴√(16711680 / catchRate))` and hands it to `trainerThrowPokeball` (`encounter-pokemon-utils.ts:433`), which asks for **three** passes of `randSeedInt(65536) < ballTwitchRate` before it locks — so P(catch) = `min(1, twitch / 65536)³`, with no HP term, no status or shiny multiplier and no critical capture, unlike `AttemptCapturePhase` (§20). A rate of 255 or more makes the twitch rate exceed 65536, so the catch is certain. Flee is `((255² − rate²) / 255 / 2) × the same modifier on fleeStage` — off the **species** catch rate, never the modified one — rolled against `randSeedInt(256)` at the end of every turn that is not a catch and not a run, a failed ball included (`:508`–`:517`). Bait: catch +2 always, flee +1 at 80 % (`randSeedInt(10) >= 8` refuses); mud: flee −2 always, catch −1 at 80 % (`:189`–`:246`) · leave | the mons are **not** on `getSeedOffset()`: `summonSafariPokemon` sows its own fork at `waveIndex × 1000 × safariPokemonRemaining` (`:322`), starter cost ≤ 5, non-legendary, with doubled shiny and hidden-ability rolls |
