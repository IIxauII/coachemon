...
- Status plays in a **double** (#262): one exchange of the field, not two 1-v-1s.
  - A slot carries at most `STATUS_CANDS` (2) status plays into the field search, best first by `turn.benefit` — the game's own score for the move against that foe. A self-targeted play is carried once, not once per foe.
