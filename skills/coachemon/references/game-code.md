...
- Status moves as this turn's action (§14). Each status move `statusPlay` can price (setup, a status on the foe, a heal, a hazard in a trainer battle, a typing written onto the foe) is turn 1 of an `exchange` in which we deal nothing.
  ...
  - Scored like a depth-2 line minus `STATUS_COST` (0.2) — in a **single battle**. A double is priced differently (below), because `follow()`'s 1-v-1 exchange would weigh the effect against the whole of one foe's damage where the field splits it.
- Status plays in a **double** (#262): one exchange of the field, not two 1-v-1s.
  - A slot carries at most `STATUS_CANDS` (2) status plays into the field search, best first by **effect value**: `turn.benefit` — the game's own score for the move against that foe — times how often the play works, so a sure Spore outranks a Hypnosis that misses two times in five. A self-targeted play is carried once, not once per foe.
  - A candidate has no standalone score: the field search prices it. Each play lands or it doesn't, so a field is the weighted mix over which of them landed; in each branch the effect is written with `turn.assuming` **before the partner slot is scored**, so Spore in slot 0 changes what slot 1's option is worth.
  - On that state: the slot that spent its turn is worth what its own best move is worth there, one turn later; the partner's own pick is re-scored there; and `joint` counts only the hits that still happen.
  - `getNextMove` picks a move whether or not the mon is asleep, so a written-on sleep changes no distribution. The attempts it costs ride separately, as `foeCurve`'s `act` / `exchange`'s `foeAct`: index 0 is this turn's attempt (cancelled by `skip0` when we move first), and every later one is #74's `foeAct`. `joint` discounts a silenced foe's `danger` and its chance of acting first by the same share.
  - Bounded to this turn: `options` under a hypothesis runs no depth 2 and offers no status plays of its own.
  - The turn a status play spends keeps its score below an attack's by construction, so it is not read as the slot losing its trade when the stay margin asks which fields are failing.
...
| Safari Zone | money ≥ ×2 (`:57`) | pay ×2 → a **continuous encounter**: three wild mons in turn, each with ball / bait / mud / run as `overrideOptions`. Catch rate is `round(speciesCatchRate × 1.5 × (2 + max(catchStage, 0)) / (2 − min(catchStage, 0)))`, and that rate is **not** a probability: `throwPokeball` turns it into `ballTwitchRate = round(1048560 / ⁴√(16711680 / catchRate))` and hands it to `trainerThrowPokeball` (`encounter-pokemon-utils.ts:433`), which asks for **three** passes of `randSeedInt(65536) < ballTwitchRate` before it locks — so P(catch) = `min(1, twitch / 65536)³`, with no HP term, no status or shiny multiplier and no critical capture, unlike `AttemptCapturePhase` (§20). A **modified** rate of 256 or more puts the twitch rate over 65536, so the catch is certain and no flee roll follows; 255 lands at 65535/65536, which is 99.995 %, and at stage 0 the first certain species rate is 171 (×1.5 → 257). Flee is `((255² − rate²) / 255 / 2) × the same modifier on fleeStage` — off the **species** catch rate, never the modified one — rolled against `randSeedInt(256)` at the end of every turn that is not a catch and not a run, a failed ball included (`:508`–`:517`). Bait: catch +2 always, flee +1 at 80 % (`randSeedInt(10) >= 8` refuses); mud: flee −2 always, catch −1 at 80 % (`:189`–`:246`) · leave | the mons are **not** on `getSeedOffset()`: `summonSafariPokemon` sows its own fork at `waveIndex × 1000 × safariPokemonRemaining` (`:322`), starter cost ≤ 5, non-legendary, with doubled shiny and hidden-ability rolls |
...
| Encounter | Gate | Options, as the source does them | Forked before an `await` |
|---|---|---|---|
| Training Session | party ≥ 2 (`:48`), no flee | pick a member; it leaves the party and fights you as a boss **of itself** — same level, form, IVs, moveset and held items (`getEnemyConfig`, `:374`), the items untransferable — then comes back with the prize. Light: `min(2 + floor(wave / 50), 5)` bars, **2 random non-maxed IVs** up by +10 under 11, +5 under 21, else +3, capped at 31 (`:104`, `:128`–`147`) · medium: `min(2 + floor(wave / 40), 6)` bars, **any nature** (`:219`, `:227`) · heavy: `min(2 + floor(wave / 30), 6)` bars and a `MYSTERY_ENCOUNTER_POST_SUMMON` tag — **+1 to every stat on entry** — for **any ability the species has**, hidden included (`:309`–`312`, `:318`) · leave. All three set `fillRemaining: true`: an ordinary reward screen on top | no draws before the party picker; the IV shuffle is in `onBeforeRewardsPhase`, after the battle |
| The Pokémon Salesman | money ≥ ×4 (`:54`) | `onInit` builds the mon and the price outright and leaves both on `misc` (`:192`): a starter-cost ≤ 5 species with a hidden ability (rerolled up to 5×), or — on `randSeedInt(100) === 0`, or when a timed event is running — a **shiny Magikarp** or an event mon with 3 extra shiny rolls (`:90`–`163`). Price is `getWaveMoneyAmount(4 × max(starterTier, 2.5) / 5)`, always the full ×4 for a shiny (`:179`–`189`). Buying `catchPokemon`s it **at level 5** with `abilityIndex` 2 (`:229`) · leave | `onInit` fork at `w`: the whole offer, readable on `misc` |
| Trash to Treasure | waves 100–180 (`:51`), party ≥ 3, once a run, no flee | investigate: a Gmax Garbodor boss, `bossSegmentModifier: 1`, level additive 0.5, `disableSwitch` — **no switching all fight** — opening with **Toxic on your lead and Stockpile on itself** (`:81`–`180`) → guaranteed Leftovers plus Rogue / Ultra / Great and `fillRemaining` · dig: **one Leftovers and one Shell Bell**, each to the first party member not already holding a full stack (`tryApplyDigRewardItems`, `:230`), and a **Black Sludge**: every shop item costs **2.5× for the rest of the run** (`:205`) | `onInit` fork at `w`: the boss's held items; the dig itself draws nothing |
| Clowning Around | waves 80–180 (`:85`) | `onInit` draws Blacephalon's ability from a 15-strong pool and its **two random types** (`:145`–`154`), leaving the ability's localized name in the `ability` dialogue token and the types on `pokemonConfigs[1].customPokemonData`. Battle: a **double** vs boss Mr. Mime and boss Blacephalon, Mr. Mime **Role Plays** that ability off it and both **Taunt** your slots (`:210`–`230`) → ordinary rewards, then the post-option phase offers that **ability onto one of your mons for good** (`handleSwapAbility`, `:438`) · item shuffle: the member with the most transferable non-berry items has its berries redrawn and its Ultra / Rogue items rerolled **inside their own tiers**, count kept (`:285`–`337`) · type shuffle: **every member's second type is redrawn**, preferring a type it already has a non-status move for — `[...new Set(offTypes)].sort()` (the default, string, sort), `randSeedShuffle`, then `pop` — else `getRandomRegularPokemonType()` (`:376`–`414`) | `onInit` fork at `w`: the ability and the types · pre ×1: the whole type shuffle, the loop being synchronous |
| The Expert Pokémon Breeder | waves 25–180 (`:137`), party ≥ 4 | the three options are **your own three lowest-friendship members**, sorted in `onInit` (`:186`); each one's egg count is worked out there too and left on `misc` — `min(max(floor((680 − BST) / 20), 1), 18)` points plus `min(max((7 − starterCost) × 2, 0), 12)`, capped at 30, first 5 points Common, then one Great per 4 and one Common per point left (`calculateEggRewardsForPokemon`, `:600`). Picking one **removes every other member from the party** and fights the breeder's three alone → those eggs, a Soothe Bell and `fillRemaining` rewards. **Losing is not the run**: `onGameOver` (`:681`) puts the party back, leaves the chosen mon fainted at 0 friendship and drops the rewards | `onInit` fork at `w`: the breeder's party and all three egg counts |
| Dark Deal | waves 30–180, party ≥ 2 (`:125`), catchable | deal: `getRandomPlayerPokemon(true, false, true)` — a random legal member **still standing**, or a random fainted legal one when only one standing member is left (`encounter-pokemon-utils.ts:91`) — is **removed from the party for good** along with its held items (`:150`–`163`). Then 5 Rogue Balls and a boss drawn from `getRandomSpeciesByStarterCost` at a tier from `randSeedInt(100)`: ≥ 65 → 6, ≥ 15 → 7, ≥ 5 → 8, else 9–10 (`:182`–`186`), sharing the taken mon's types and **carrying its held items** (`:190`). `withCatchAllowed(true)`, so the legendary can be caught · leave | pre ×1: which member it takes · option ×500: the tier roll, then the species |
| A Trainer's Test | rogue, no wave gate of its own | `onInit` draws one of five stat trainers with `randSeedInt(5)` — Cheryl, Marley, Mira, Riley, else Buck — and leaves it on `misc` and in the `statTrainerName` token (`:53`–`107`); the config is `levelAdditiveModifier: 1` over an **`ELITE_FOUR` party template, 6 mons** (`initForStatTrainer`, `trainer-config.ts:623`). Accept: fight it → an **Epic egg** plus a Relic Gold, 2 Rogue tiers and `fillRemaining` (`:158`–`172`) · decline: a **full party heal** (`PartyHealPhase`) plus a **Rare egg**, and the shop with no free reward (`:184`–`193`) | `onInit` fork at `w`: which trainer |
| Weird Dream | waves 30–140 (`:132`), party ≥ 3, once a run | `onInit` transforms the **whole team** and leaves it on `misc.teamTransformations` (`:158`): two members (shuffled, so not the first two) to a species +90–110 BST, the rest +40–50, at most one over 600 and one in 570–600 (`getTeamTransformations`, `:374`). Accept: the party is swapped, each new mon keeping the old one's **level, held items, moveset, passive, luck, Pokérus and the better of the two IV sets**, with a **neutral nature**, a redrawn ability, an extra egg move and a random second type (`postProcessTransformedPokemon`, `:511`) — then a **full party heal** and a screen of exactly a Memory Mushroom, a Rogue Ball and 4 Mints (`:226`–`236`) · battle that same team at your own levels, holding your items, bosses over 570 BST → exactly 2 Rogue / 2 Ultra / 2 Great, and `onBeforeRewards` **unlocks the passive on one random member without one, for the run** (`:308`–`334`) · refuse: **every member drops to `ceil(0.9 × level)`** (`:352`) | `onInit` fork at `w`: the species each member becomes. The ability, IVs, nature and second type are redrawn later, on the live stream |
| The Winstrate Challenge | waves 100–180 (`:48`), party ≥ 3, once a run | accept: **five trainer battles back to back** — configs pushed Vito, Vicky, Vivi, Victoria, Victor and `pop`ped, so Victor is first and Vito's five last, 13 mons in all — with **no healing between them**: `endTrainerBattleAndShowDialogue` only resets per-battle state (`:184`–`251`). Clearing all five gives a **Premium Voucher** and a screen of exactly a **Macho Brace**, its tier forced to Master (`:165`–`178`) · refuse: a **full party heal** plus a Rarer Candy (`:146`–`150`) | no draws |
