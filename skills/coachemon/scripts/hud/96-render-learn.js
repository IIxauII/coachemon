// Learn-move card (the 40-learn model): the incoming move, every slot it is weighed against, the team line and the
// call.
import { FS, badge, bar, closed, dim, h, img, line, mon, tab } from "./90-render.js";
...
export const drawLearn = m => {
  if (closed()) return [tab("🎓", mon(m.icon, m.name, 20))];
  ...
  const verdict = h("div", { color: m.verdict[1], fontWeight: "bold", marginTop: "3px" }, m.verdict[0], gain);
