...
// Whether a modifier type is an instance of the game class `name`, by walking its prototype chain: the game's classes
// are minified, so `instanceof` has no name to reach for and only the constructor names it kept can be matched. Every
// card that sorts rewards by their game class reads them through this.
...
export const rewardValue = (t, ctx, users) => {
  ...
  // (50-audit's `relearnBest`). Nothing worth relearning is worth little: the mushroom is spent on pick.
