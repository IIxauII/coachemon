...
// The panel's one storage key, and the dismissed state as it is stored: the view and the last group id together.
const PANEL_KEY = "coach-hud-panel";
const SHUT = { [PANEL_KEY]: JSON.stringify({ view: "closed", group: "act" }) };
...
// goes with it — so what survives one is exactly what the key carried.
{
  ...
  // The panel is its control, the strip, the bar, the pane and the footer — so a shut drawer is three children and
  ...
  // discovers what the coach does without hunting for it.
  {
    ...
    console.log(`== remembers · first run\n${state(el)} · ${globalThis.__hud["90-render"].openGroup()}`);
    assert.equal(state(el), "drawer");
    assert.equal(globalThis.__hud["90-render"].openGroup(), "act");
  }

  // **The old key's three values migrate**, and the key itself is dropped: two keys that can disagree is a state
  // the panel would have to arbitrate on every read. The old key never held a group, so a migrated panel opens on
  // `act` whatever view it was in.
  for (const [old, view] of [["full", "drawer"], ["mini", "strip"], ["closed", "closed"]]) {
    const el = mount(battle(), { stored: { "coach-hud-view": old } });
    console.log(`== remembers · migrate ${old}\n${state(el)} · ${store.get(PANEL_KEY)}`);
    assert.equal(state(el), view);
    assert.deepEqual(remembers(), { view, group: "act" });
  ...
  // draws the same card on `foes`, because the key carried the id and not a position.
  {
    ...
    assert.deepEqual(remembers(), { view: "drawer", group: "foes" });
    const back = mount(battle(), { expose: true, stored: Object.fromEntries(store) });
    console.log(`== remembers · reload on foes\ntabs ${back.kids[2].children.map(flat).join(" | ")}\nopen ${globalThis.__hud["90-render"].openGroup()}`);
    assert.equal(globalThis.__hud["90-render"].openGroup(), "foes");
    assert.deepEqual(back.kids[2].children.map(t => t.style.fontWeight), ["normal", "bold", "normal"]);
    ...
    console.log(`== remembers · the fallback\ntabs ${moved.kids[2].children.map(flat).join(" | ")}\nopen ${globalThis.__hud["90-render"].openGroup()}`);
    assert.equal(globalThis.__hud["90-render"].openGroup(), "act");
    assert.deepEqual(remembers(), { view: "drawer", group: "act" }, "the move is written back, so it survives too");
    const returned = mount(battle(), { expose: true, stored: Object.fromEntries(store) });
    assert.equal(globalThis.__hud["90-render"].openGroup(), "act", "and no jump back when the group returns");
    assert.ok(returned.kids[2].children.map(flat).includes("Foes"), "the tab is there to be picked again");
  }

  // **Shutting the drawer keeps the strip**, so a whole run can be watched on one line. The strip is the drawer's
  // handle: it is what shuts the drawer, and what brings it back on the group it was left on.
  {
    const el = mount(battle(), { expose: true });
    click(el.kids[2].children[1]);
    click(el.kids[1]);
    ...
    assert.deepEqual(remembers(), { view: "strip", group: "foes" }, "and the group it was on is still remembered");
    ...
    assert.equal(globalThis.__hud["90-render"].openGroup(), "foes", "on the group the drawer was left on");
  }
  ...
  // way back, so a dismissal is not also a reset.
  {
    ...
    click(el.kids[0]);
    assert.equal(state(el), "drawer");
    assert.equal(globalThis.__hud["90-render"].openGroup(), "foes", "reopening restores the drawer that was there");
  }
  ...
  for (const raw of ["", "{", JSON.stringify({ view: "mini", group: "elsewhere" })]) {
