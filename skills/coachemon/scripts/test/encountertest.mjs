...
// Great tier: Mysterious Challengers' three trainer fights by level bump and team size, Slumbering Snorlax's thief
...
// A trainer-battle config: no species to read, only who it is and the level bump it carries.
const trainerCfg = (name, levelAdditiveModifier) => ({ trainerConfig: { name }, levelAdditiveModifier });
...
{
  const challengers = extra => ({ type: 0, tier: GREAT, labels: ["Normal", "Hard", "Brutal"], options: [option(), option(), option()],
    configs: [trainerCfg("Youngster Joey", 0), trainerCfg("Ace Trainer May", 1), trainerCfg("Leader Brock", 1.5)], ...extra });
  // Wave 30: bumps of +0, +3, +5, and teams of (theirs), 3, 6 against three standing mons.
  ...
  assert.equal(m.options[1].why, "+3 levels, 3 mons vs your 3");
  assert.deepEqual(verdicts(m), ["ok", "take", "avoid"], "+5 levels is hard, +3 against an even team isn't");
  // Late enough and every fight is hard, so the mildest one is the call.
...
// ---- 20. Bug-Type Superfan: the four tutor moves are the option fork's first four draws, one per pool.
{
  const superfan = (party, extra = {}) => ({ type: 26, tier: GREAT, labels: ["Battle", "Show", "Give"],
    ...
    configs: [trainerCfg("Bug-Type Superfan", 0)], party, ...extra });
