...
- [Button and UiMode enum tables](issues/01-enum-tables.md): `Button` (18 members, `UP`=0…`DEV_CUSTOM`=17) in `src/enums/buttons.ts`; `UiMode` (48 members, `MESSAGE`=0…`ALERT_MODAL`=47) in `src/enums/ui-mode.ts`. Both read at `e4e9b53` / tag `v1.12.0.11` and cross-verified against the live production bundle. **The repo's default branch is `beta`, not `main`** — `main` is what deploys to pokerogue.net, and beta's table already differs, so an unpinned fetch generates a wrong table. Pin via `game.config.gameVersion` → `v${that}`, a real upstream tag; `gameInfoVersion` is the `gameInfo` payload schema version, not a build id. `Button` is append-only since 2024; `UiMode` renumbers by mid-list insert roughly every five months. Codegen must parse TS with the compiler API (package is private, not on npm; `BiomeId`/`PartyUiMode` are `as const` objects with non-contiguous values). Ten more small enums worth generating; `MoveId`/`SpeciesId`/`AbilityId` are not, `gameInfo` already returns those as strings. Free runtime drift checks: `inputController.configs["default"].settings` exposes live `Button` values, and `ui.handlers.length` equals the `UiMode` member count. Findings on branch `research/enum-tables` at `.scratch/pokerogue-mcp-v1/research/01-enum-tables.md`.
...
- **Enum codegen script** — where the generated module lands, and how the two free runtime drift checks are wired in. Pinning is settled: resolve `game.config.gameVersion` to the `v…` tag, never a default-branch fetch.
