// PROTOTYPE (#161) — launches a throwaway Firefox profile and installs the probe builds as temporary add-ons over
// the Remote Debugging Protocol (what `web-ext run` does, but web-ext takes one source dir per run). The RDP
// connection is closed once the installs are done, so no debugger is attached while the probes idle.
//
// Usage: node firefox.mjs <Firefox.app path> <profile dir>
...
for (const variant of variantsFor("firefox")) {
  const r = await request({ to: root.addonsActor, type: "installTemporaryAddon", addonPath: path.join(here, "probe", "dist", "firefox", variant) });
  console.log(variant, JSON.stringify(r.addon ?? r));
}
sock.end();
console.log("RDP closed; add-ons stay installed until Firefox exits");
process.exit(0);
