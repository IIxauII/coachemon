...
- **The floor is 142, not the 128 the page world would need.** `data_collection_permissions` is mandatory on AMO — since 2025-11-03 for new extensions, and for every extension through the first half of 2026 **[doc]** — and the key was understood by Firefox 140 and Firefox for Android 142. The linter warns once per target whose floor predates the key, and with no `gecko_android` key it checks Android against this same floor, so 128 drew both warnings — desktop and Android ([#380](https://github.com/IIxauII/coachemon/issues/380)). 142 clears both. It is above the 140 the desktop warning asked for, because Android is the binding one and a single floor serves both: addons-linter on the built store artifact drops from three warnings to one **[live]**, the survivor being §6's declared `import()` — which [#381](https://github.com/IIxauII/coachemon/issues/381) then removed, leaving the artifact at zero (§5.6).
